...
class Book:
	...
	def  handleEvent(self, event, i):
		...
		#self.logger.info('{} New Event Type {}'.format(i, event.type))
		# Handle Add Order Messages
		if event.type == "A":
			#self.logger.info('{} Add Order No MPID Attribution'.format(i))
			self.addOrder(event, mpid=False)
		elif event.type == "F":
			#self.logger.info('{} Add Order With MPID Attribution'.format(i))
			self.addOrder(event, mpid=True)
		# # Handle Modify Order Messages
		elif event.type == "E":
			#self.logger.info('{} Order Execution'.format(i))
			self.orderExecution(event)
		elif event.type == "C":
			#self.logger.info('{} Order Execution With Price'.format(i))
			self.orderExecution(event, with_price=True)
		elif event.type == "X":
			#self.logger.info('{} Order Cancel'.format(i))
			self.orderCancel(event)
		elif event.type == "D":
			#self.logger.info('{} Order Delete'.format(i))
			self.orderDelete(event)
		elif event.type == "U":
			#self.logger.info('{} Order Replace'.format(i))
			self.orderReplace(event)
		# Handle Trade Messages
		elif event.type == "P":
			#self.logger.info('{} Trade'.format(i))
			self.tradeEvent(event)
		elif event.type == "Q":
			#self.logger.info('{} Cross Trade'.format(i))
			self.crossTradeEvent(event)
		
		# # aggregate info for use later 
		self.event_times.append(self.event_time)
		self.book_snapshot.append([self.getXLevels(2), self.event_time])
	...
	def addOrder(self, event, mpid):
		...
		if event.type == "U":
			new_order.life.append([self.event_time, event.shares_replaced, event.type, event.price_replaced]) 
		else:
		...
		# add to new submissions and flag MPID attribution
		self.submissions.append([self.event_time, new_order.id, new_order.price, new_order.shares, new_order.direction, mpid])

	def orderExecution(self, event, with_price=None):
		"""
		Executes an order.
		...
		order_to_execute = self.orders.get(event.order_reference_number)
		shares_traded = event.executed_shares
		# check if order exists
		if order_to_execute is not None:
			# update order life
			order_to_execute.life.append([self.event_time, event.executed_shares, event.type, event.execution_price])
			...
			# Add to trades list fi printable
			if with_price == True:
				if event.printable == 1:
					#self.logger.info("Trade is printable - adding to time and sales")
					...
					#self.logger.info("Trade is not printable")
					pass
			...
			#self.logger.info("ID {} does not exist".format(event.order_reference_number))
			pass
	...
	def orderCancel(self, event):
		...
		order_to_cancel = self.orders.get(event.order_reference_number)
		shares_to_subtract_from_limit_total = event.cancelled_shares
		# check if order exists
		if order_to_cancel is not None:
			# update order life 
			order_to_cancel.life.append([self.event_time, event.shares, event.type, event.price])
			if (order_to_cancel.shares != 0) and (order_to_cancel.shares != event.cancelled_shares):
	...
	def orderDelete(self, event, specific_id = None):
			...
			"""
			# deal with different IDs as a result of replace messages
			# we cannot use the standard order reference feild for all deletes
			if specific_id is not None:
				order_to_delete = self.orders.get(specific_id)
				order_to_delete.shares = event.shares_replaced
			else:
				order_to_delete = self.orders.get(event.order_reference_number)
	...
	def orderReplace(self, event):
		"""
		Replace an existing limit order with a new one
		...
		######
		"""
		Type U messages will not nescesarily provide order_reference_number 
		therefore we have to check the original_order_reference_number feild 

		check 'original order reference number'
		- what if original price and num shares are NaN?


		"""
		# get the original order ID to check if it exists
		original_order = self.orders.get(event.original_order_reference_number)
		if original_order is not None:
			#self.logger.info("Replacing ID {} with ID {}".format(event.original_order_reference_number, event.new_order_reference_number))
			#self.logger.info("Price change from {} to {}".format(event.price_replaced, event.price))
			#self.logger.info("Shares change from {} to {}".format(event.shares_replaced, event.shares))

			### if there are no other variables associated then we just swith the order numbers in the original order
			if event.shares_replaced == None and event.price_replaced == None:
				original_order.id = event.new_order_reference_number
				self.orders[int(original_order.id)] = original_order
				self.orders.pop(event.original_order_reference_number, None)
			else:
				### here we have to delete the original order and add a new one at the new price with the new shares and price values 
				### we can handle the addition within the order object class - there we account for order type U 
				### this will handle the "new order reference number" problem 
				if event.attribution == None:
					self.addOrder(event, mpid=False)
				else:
					self.addOrder(event, mpid=True)
				self.orderDelete(event, event.original_order_reference_number)
		else:
			#self.logger.info("Original ID {} does not exist".format(event.order_reference_number))
			if (event.shares_replaced is not None) and (event.price_replaced is not None):
				if event.attribution == None:
					self.addOrder(event, mpid=False)
				else:
					self.addOrder(event, mpid=True)
		self.replacements.append([self.event_time, original_order])

	def tradeEvent(self, event):
		"""
		This is a hidden execution event - no effect on the order book, but to be considered for time & sales
		"""
		self.hidden_executions.append([self.event_time, event.price, event.shares, event.direction])

	def crossTradeEvent(self, event):
		"""
		cross trade event message
		"""
		pass
	...
	def updateNbbo(self):
		...
		"""
		b = self.buy.inOrderReversalMax(limit=1)
		o = self.sell.inOrderTraversalMax(limit=1)  
		if len(b) == 0:
			self.best_bid = None
		else:
			self.best_bid = b[0].limit_price
			self.best_bids.append(self.best_bid)
		if len(o) == 0:
			self.best_offer = None
		else:
			self.best_offer = o[0].limit_price
			self.best_offers.append(self.best_offer)
	...
	def getXLevels(self, x):
		...
		"""
		b = self.buy.inOrderReversalMax(limit=x)
		o = self.sell.inOrderTraversalMax(limit=x)
	...
	def getDepth(self):
		b = len(self.buy.inOrderTraversal())
		o = len(self.sell.inOrderTraversal())
		return [b, o]
	
	# def getTrimDepth(self):
	# 	b = self.buy.inOrderTraversalMaxFive()
	# 	o = self.sell.inOrderTraversalMaxFive()
	# 	if b and o :
	# 		return [len(b), len(o)]
	...
	def getXorderqueues(self, x):
		...
		"""
		book_levels = self.getXLevelspricesonly(x)
		bids = book_levels[0]
		asks = book_levels[1]
	...
	def getOrdersatlimit(self, limit):
		...
		limit_to_get_queue_buy = self.buy.getLimit(limit)
		limit_to_get_queue_sell = self.sell.getLimit(limit)
		if limit_to_get_queue_buy is not None:
			return limit_to_get_queue_buy.order_queue.getOrderqueue()
		elif limit_to_get_queue_sell is not None:
			return limit_to_get_queue_sell.order_queue.getOrderqueue()
	...
	# 	self.sell.TreePrinter(self.sell)
	# 	self.buy.TreePrinter(self.buy)
