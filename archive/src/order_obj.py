...
class Order:
	...
	def __init__(self, data):
		...
		# for regular order additon (type A) we can use regular fields
		# for addition by replacement (type U) we must use replcament feilds
		if (data.type == "A") or (data.type == "F"):
			self.entryTime = copy.copy(data.time)
			self.id = copy.copy(data.order_reference_number)
			self.shares = copy.copy(data.shares)
			self.price = copy.copy(data.price)
			self.direction = copy.copy(data.direction)
			self.attribution = copy.copy(data.attribution)
			self.next = None
			self.prev = None
			self.life = []
			...
			self.entryTime = copy.copy(data.time)
			self.id = copy.copy(data.new_order_reference_number)
			self.shares = copy.copy(data.shares)
			self.price = copy.copy(data.price)
			self.direction = copy.copy(data.direction)
			self.attribution = copy.copy(data.attribution)
	...
	def setOrderTypeU(self, order, event):
		"""
